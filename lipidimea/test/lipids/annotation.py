...
class TestSumCompLipidDB(TestCase):
...
class TestRemoveLipidAnnotations(TestCase):
...
class TestAnnotateLipidsSumComposition(TestCase):
...
class TestFilterAnnotationsByRTRange(TestCase):
...
class TestUpdateLipidIDsWithFragRules(TestCase):
